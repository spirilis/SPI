...
{
	volatile uint8_t p1dir_save=P1DIR, p1out_save=P1OUT, p1ren_save=P1REN;
	uint8_t retb=0;

	if (spimode == SPI_MODE_0) {
		P1REN &= ~(BIT5 | BIT6 | BIT7);
		P1OUT &= ~(BIT5 | BIT6 | BIT7);
		P1DIR = (p1dir_save & ~(BIT5 | BIT6 | BIT7)) | BIT5 | BIT7;
	...
	if (spimode == SPI_MODE_2) {
		P1REN &= ~(BIT5 | BIT6 | BIT7);
		P1OUT = (P1OUT & ~(BIT5 | BIT6 | BIT7)) | BIT5;
		P1DIR = (p1dir_save & ~(BIT5 | BIT6 | BIT7)) | BIT5 | BIT7;
		P1SEL2 &= ~(BIT5 | BIT6 | BIT7);
		P1SEL &= ~(BIT5 | BIT6 | BIT7);
		//UCB0CTL1 |= UCSWRST;
...
{
	uint16_t retw=0;
	uint8_t spi_mode_snoop = UCB0CTL0 & SPI_MODE_MASK;
	uint8_t spi_bitorder_snoop = UCB0CTL0 & UCMSB;
	...
	UCB0TXBUF = (uint8_t) (_data & 0x00FF); // setting TXBUF clears the TXIFG flag

	while (!(UC0IFG & UCB0RXIFG))
		; // wait for an rx character?

	retw |= UCB0RXBUF; // reading clears RXIFG flag
